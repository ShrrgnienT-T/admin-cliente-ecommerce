...
@section('content')
    <div class="container">
        ...
        <h2 class="mb-4 fw-bold" style="color:#fff;text-shadow:1px 1px 6px #000, 0 0 2px #ff003c;">Roupas à Venda
        </h2>
        <div class="row mb-3">
            <div class="col">
                <a href="{{ route('roupas.venda', ['subcategoria' => 'Masculino']) }}"
                    class="btn btn-outline-danger btn-sm fw-bold">Masculino</a>
                <a href="{{ route('roupas.venda', ['subcategoria' => 'Feminino']) }}"
                    class="btn btn-outline-danger btn-sm fw-bold">Feminino</a>
                <a href="{{ route('roupas.venda', ['subcategoria' => 'Infantil']) }}"
            ...
            @foreach ($roupas as $roupa)
                <div class="col-md-4 mb-4">
                    <div class="card h-100 border-0 shadow-lg"
                        ...
                        <img src="{{ $roupa->imagem }}" class="card-img-top" alt="Imagem do produto"
                            style="height:180px;object-fit:cover;">
                        <div class="card-body">
                            <h5 class="card-title fw-bold" style="text-shadow:1px 1px 4px #000;">{{ $roupa->nome }}</h5>
                            <p class="card-text">Tamanho: {{ $roupa->tamanho }} | Cor: {{ $roupa->cor }}</p>
                            <p class="card-text">Subcategoria: {{ $roupa->subcategoria }}</p>
                            ...
                            <form method="POST" action="{{ route('roupas.adicionarAoCarrinho', $roupa) }}">
                                @csrf
